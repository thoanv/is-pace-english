...
use App\Repositories\RecruitmentRepository as Repo;
use Carbon\Carbon;
...
class RecruitmentService extends BaseService
{
    ...
    public function createData($request)
    {
        DB::beginTransaction();
        try {
            $data = $request->only('title','category_id','vi_tri','bang_cap','thu_nhap','hinh_thuc_lam_viec','noi_lam_viec','kinh_nghiem', 'lam_viec','content', 'image');
    ...
    public function updateData($request, $item)
    {
        DB::beginTransaction();
        try {
            $data = $request->only('title','vi_tri','category_id','bang_cap','thu_nhap','hinh_thuc_lam_viec','noi_lam_viec','kinh_nghiem', 'lam_viec','content', 'image');
            $data['status'] = isset($request['status']) ? CommonEnum::ACTIVATED : CommonEnum::UNACTIVATED;
            $data['date_publish'] = Carbon::parse($request->date_publish)->format('Y-m-d H:i:s');
            $data['updated_by'] = Auth::id();
    ...
    public function getNewPostOtherSlug($post){
        return $this->repo->getNewPostOtherSlug($post);
    }
